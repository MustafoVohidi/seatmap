...
interface TriangleSeat extends Seat {
  rowID: number;
  radius: number;
  colID: number;
...
export function useCreateTriangle(blocks: any) {
  const store = useStore();
  let triangleSector: any = reactive({});
  ...
  onUpdated(() => {
    ...
    triangleSector.value = returnBlock();
  });
