...
interface TrapezoidSeat extends Seat {
  rowID: number;
  radius: number;
  colID: number;
...
export function useCreateTrapezoid(blocks: any) {
  const store = useStore();
  let trapezoidSector: any = reactive({});
  ...
  function returnBlock() {
    ...
    let initialBlock: TrapezoidZone = {
      x: 0,
      y: 0,
      width: 1000,
      height: 500,
  ...
  onUpdated(() => {
    ...
    trapezoidSector.value = returnBlock();
  });
